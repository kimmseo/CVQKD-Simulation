...
const gchar    *SECOND_SAT_FIELD_HINT[SECOND_SAT_FIELD_NUMBER] = {
...
static void gtk_second_sat_destroy(GtkWidget * widget)
{
    ...
    if (sat != NULL)
        g_key_file_set_integer(ssat->cfgdata, MOD_CFG_SECOND_SAT_SECTION,
                               MOD_CFG_SECOND_SAT_SELECT, sat->tle.catnr);

    (*GTK_WIDGET_CLASS(parent_class)->destroy) (widget);
}

static void gtk_second_sat_class_init(GtkSecondSatClass * class,
				      gpointer class_data)
{
    ...
    widget_class->destroy = gtk_second_sat_destroy;
    parent_class = g_type_class_peek_parent(class);
}

static void gtk_second_sat_init(GtkSecondSat * list,
				gpointer g_class)
{
    (void)list;
    (void)g_class;
...
/* Update a field in the GtkSECONDSat view. */
static void update_field(GtkSecondSat * ssat, guint i)
{
    sat_t          *sat;
    gchar          *buff = NULL;
    ...
    case SECOND_SAT_FIELD_SSP:
        ...
        break;
    case SECOND_SAT_FIELD_FOOTPRINT:
        if (sat_cfg_get_bool(SAT_CFG_BOOL_USE_IMPERIAL))
        {
...
{
    GtkSecondSat   *second_sat = GTK_SECOND_SAT(user_data);
    sat_t          *sat = SAT(value);

    (void)key;

    second_sat->sats = g_slist_insert_sorted(second_sat->sats, sat,
...
{
    GtkSecondSat   *ssat = GTK_SECOND_SAT(data);
    guint           i =
...
/* Second sat options menu */
static void gtk_second_sat_popup_cb(GtkWidget * button, gpointer data)
{
    GtkSecondSat   *second_sat = GTK_SECOND_SAT(data);
    GtkWidget      *menu;
    GtkWidget      *menuitem;
    GtkWidget      *label;
    GSList         *group = NULL;
    ...
    n = g_slist_length(second_sat->sats);
    ...
    g_object_set_data(G_OBJECT(menuitem), "qth", second_sat->qth);
    ...
    add_pass_menu_items(menu, sat, second_sat->qth, &second_sat->tstamp, data);
    ...
    for (i = 0; i < n; i++)
    {
        sati = SAT(g_slist_nth_data(second_sat->sats, i));
        ...
        g_signal_connect_after(menuitem, "activate",
                               G_CALLBACK(select_satellite), second_sat);
        gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
    }
...
void gtk_second_sat_reload_sats(GtkWidget * second_sat, GHashTable * sats)
{
    /* free GSlists */
    g_slist_free(GTK_SECOND_SAT(second_sat)->sats);
    GTK_SECOND_SAT(second_sat)->sats = NULL;

    /* reload satellites */
    g_hash_table_foreach(sats, store_sats, second_sat);
}

/*
 * Reload configuration.
 * 
 * @param widget The GtkSecondSat widget.
                           ...
                           GHashTable * sats, qth_t * qth, gboolean local)
{
    ...
    GTK_SECOND_SAT(widget)->cfgdata = newcfg;

    /* get visible fields from new configuration */
    fields = mod_cfg_get_int(newcfg,
                             MOD_CFG_SECOND_SAT_SECTION,
                             MOD_CFG_SECOND_SAT_FIELDS,
                             SAT_CFG_INT_SECOND_SAT_FIELDS);

    if (fields != GTK_SECOND_SAT(widget)->flags)
        GTK_SECOND_SAT(widget)->flags = fields;

    /* if this is a local reconfiguration sats may have changed */
    if (local)
        gtk_second_sat_reload_sats(widget, sats);

    /* QTH may have changed too since we have a default QTH */
    GTK_SECOND_SAT(widget)->qth = qth;

    /* get refresh rate and cycle counter */
    GTK_SECOND_SAT(widget)->refresh = mod_cfg_get_int(newcfg,
                                                      MOD_CFG_SECOND_SAT_SECTION,
                                                      MOD_CFG_SECOND_SAT_REFRESH,
                                                      SAT_CFG_INT_SECOND_SAT_REFRESH);
    GTK_SECOND_SAT(widget)->counter = 1;
}

/* Select new satellite */
void gtk_second_sat_select_sat(GtkWidget * second_sat, gint catnum)
{
    GtkSecondSat   *ssat = GTK_SECOND_SAT(second_sat);
    sat_t          *sat = NULL;
    gchar          *title;
...
void gtk_second_sat_update(GtkWidget * widget)
{
    ...
    if ((ssat == NULL) || !IS_GTK_SECOND_SAT(ssat))
    {
        ...
        sat_log_log(SAT_LOG_LEVEL_ERROR,
                    _("%s: Invalid GtkSecondSat!"), __func__);
        return;
    }
    ...
    else
    {
        ...
        for (i = 0; i < SECOND_SAT_FIELD_NUMBER; i++)
        {
            if (ssat->flags & (1 << i))
                update_field(ssat, i);
	    else
...
GType gtk_second_sat_get_type()
{
    static GType    gtk_second_sat_type = 0;

    if (!gtk_second_sat_type)
    {
        static const GTypeInfo gtk_second_sat_info = {
            sizeof(GtkSecondSatClass),
            NULL,               /* base_init */
            NULL,               /* base_finalize */
            (GClassInitFunc) gtk_second_sat_class_init,
            NULL,               /* class_finalize */
            NULL,               /* class_data */
            sizeof(GtkSecondSat),
            5,                  /* n_preallocs */
            (GInstanceInitFunc) gtk_second_sat_init,
            NULL
        };

        gtk_second_sat_type = g_type_register_static(GTK_TYPE_BOX,
                                                     "GtkSecondSat",
                                                     &gtk_second_sat_info, 0);
    }

    return gtk_second_sat_type;
}

GtkWidget      *gtk_second_sat_new(GKeyFile * cfgdata, GHashTable * sats,
                                   qth_t * qth, guint32 fields)
{
    GtkWidget      *widget;
    GtkSecondSat   *second_sat;
    ...
    widget = g_object_new(GTK_TYPE_SECOND_SAT, NULL);
    ...
    if (fields > 0)
        second_sat->flags = fields;
    else
        second_sat->flags = mod_cfg_get_int(cfgdata,
                                            MOD_CFG_SECOND_SAT_SECTION,
                                            MOD_CFG_SECOND_SAT_FIELDS,
                                            SAT_CFG_INT_SECOND_SAT_FIELDS);
    ...
    /* get refresh rate and cycle counter */
    second_sat->refresh = mod_cfg_get_int(cfgdata,
                                          MOD_CFG_SECOND_SAT_SECTION,
                                          MOD_CFG_SECOND_SAT_REFRESH,
                                          SAT_CFG_INT_SECOND_SAT_REFRESH);
    second_sat->counter = 1;

    /* get selected catnum if available */
    selectedcatnum = mod_cfg_get_int(cfgdata,
                                     MOD_CFG_SECOND_SAT_SECTION,
                                     MOD_CFG_SECOND_SAT_SELECT,
                                     SAT_CFG_INT_SECOND_SAT_SELECT);
    /* popup button */
    second_sat->popup_button =
        gpredict_mini_mod_button("gpredict-mod-popup.png",
                                 _("Satellite options / shortcuts"));
    g_signal_connect(second_sat->popup_button, "clicked",
                     G_CALLBACK(gtk_second_sat_popup_cb), widget);

    hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
    gtk_box_pack_start(GTK_BOX(hbox), second_sat->popup_button,
                       FALSE, FALSE, 0);
    ...
    g_free(title);
    g_object_set(second_sat->header, "xalign", 0.0f, "yalign", 0.5f, NULL);
    gtk_box_pack_start(GTK_BOX(hbox), second_sat->header, TRUE, TRUE, 10);

    gtk_box_pack_start(GTK_BOX(widget), hbox, FALSE, FALSE, 0);

    /* create and initialise table  */
//    second_sat->table = gtk_table_new(SECOND_SAT_FIELD_NUMBER, 3, FALSE);
    second_sat->table = gtk_grid_new();
    gtk_container_set_border_width(GTK_CONTAINER(second_sat->table), 5);
    gtk_grid_set_row_spacing(GTK_GRID(second_sat->table), 0);
    gtk_grid_set_column_spacing(GTK_GRID(second_sat->table), 5);

    /* create and add label widgets */
    for (i = 0; i < SECOND_SAT_FIELD_NUMBER; i++)
    {
        if (second_sat->flags & (1 << i))
        {
            label1 = gtk_label_new(_(SECOND_SAT_FIELD_TITLE[i]));
            g_object_set(label1, "xalign", 1.0f, "yalign", 0.5f, NULL);
            gtk_grid_attach(GTK_GRID(second_sat->table), label1, 0, i, 1, 1);

            label2 = gtk_label_new("-");
            g_object_set(label2, "xalign", 0.0f, "yalign", 0.5f, NULL);
            gtk_grid_attach(GTK_GRID(second_sat->table), label2, 2, i, 1, 1);
            second_sat->labels[i] = label2;

            /* add tooltips */
            gtk_widget_set_tooltip_text(label1, _(SECOND_SAT_FIELD_HINT[i]));
            gtk_widget_set_tooltip_text(label2, _(SECOND_SAT_FIELD_HINT[i]));

            label1 = gtk_label_new(":");
            gtk_grid_attach(GTK_GRID(second_sat->table), label1, 1, i, 1, 1);
        }
        else
        {
            second_sat->labels[i] = NULL;
        }
    }

    /* create and initialise scrolled window */
    second_sat->swin = gtk_scrolled_window_new(NULL, NULL);
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(second_sat->swin),
                                   GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
    gtk_container_add(GTK_CONTAINER(second_sat->swin), second_sat->table);
    gtk_box_pack_end(GTK_BOX(widget), second_sat->swin, TRUE, TRUE, 0);
    gtk_widget_show_all(widget);

    if (selectedcatnum)
        gtk_second_sat_select_sat(widget, selectedcatnum);
