...
static GtkWidget *check[TWO_SAT_FIELD_NUMBER];
static guint    startflags;
static guint    flags;
static gboolean dirty = FALSE;
static gboolean reset = FALSE;

extern const gchar *TWO_SAT_FIELD_TITLE[];
extern const gchar *TWO_SAT_FIELD_HINT[];

// User pressed cancel. Any changes to config must be cancelled.
void sat_pref_two_sat_cancel(GKeyFile * cfg)
{
    (void) cfg;
...
void sat_pref_two_sat_ok(GKeyFile * cfg)
{
    if (dirty)
    {
        if (cfg != NULL)
        {
            g_key_file_set_integer(cfg,
                                   ...
                                   MOD_CFG_TWO_SAT_FIELDS, flags);
        }
        else
        {
            sat_cfg_set_int(SAT_CFG_INT_TWO_SAT_FIELDS, flags);
        }
    }
    else if (reset)
    {
        if (cfg != NULL)
        {
            ...
            g_key_file_remove_key(cfg,
                                  MOD_CFG_TWO_SAT_SECTION,
                                  MOD_CFG_TWO_SAT_FIELDS, NULL);
        }
        else
        {
            // global mode - reset value
            sat_cfg_reset_int(SAT_CFG_INT_TWO_SAT_FIELDS);
        }
    }
...
static void reset_cb(GtkWidget * button, gpointer cfg)
{
    ...
    if (cfg == NULL)
    {
        ...
        flags = sat_cfg_get_int_def(SAT_CFG_INT_TWO_SAT_FIELDS);
    }
    else
    {
        // local mode, get global value
        flags = sat_cfg_get_int(SAT_CFG_INT_TWO_SAT_FIELDS);
    }
...
GtkWidget *sat_pref_two_sat_create(GKeyFile * cfg)
{
    GtkWidget *table;
    GtkWidget *vbox;
    GtkWidget *label;
    ...
    {
        check[i] =
            gtk_check_button_new_with_label(TWO_SAT_FIELD_TITLE[i]);
        gtk_widget_set_tooltip_text(check[i], TWO_SAT_FIELD_HINT[i]);
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(check[i]),
                                     flags & (1 << i));
