...
void last_simpson_entry(GHashTable *history, char *key, simpson_intg *val, gdouble time_step);
...
void generate_link_capacities(
    ...
    value is struct simpson_rule pointer
    */
    ...
    //ToDo: deal with link caps that didnt finish in time
...
void update_link_cap(GList *sats, GHashTable *history, gdouble limit, gdouble time_step) {
    ...
    for (current = sats; current != NULL; current = current->next) {
        ...
        for (check = current->next; check != NULL; check = check->next) {
            ...
            gdouble cur_value = inter_sat_link(current->data, check->data) * (60 * time_step / 1000);
            ...
            if (tfr_value  == NULL && cur_value > limit) {
                ...
                last_simpson_entry(history, tfr_key, tfr_value, time_step);
            }
...
void update_simpson_entry(simpson_intg *tfr_val, gdouble cur_val, gdouble time_step) {
    //start of simpson integration 
    if (tfr_val->cumulative == 0) {
        tfr_val->cumulative = (time_step/3) * tfr_val->last;
        tfr_val->last_even = FALSE;
    
    //last value was even, append to cumulative
    } else if(tfr_val->last_even) {
...
//ToDo: add code to check if it's even length and add an extra measurement if not
void last_simpson_entry(
    ...
    printf("transfer key: %s, capacity: %f\n", key, val->cumulative);
