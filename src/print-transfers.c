...
void new_simpson_entry(GHashTable *history, char *tfr_key, gdouble tfr_value, gdouble time);
void update_simpson_entry(simpson_intg *tfr_val, gdouble cur_val, gdouble time_step);
void last_simpson_entry(GHashTable *history, char *key, simpson_intg *val, gdouble cur_val, gdouble time, gdouble time_step);
char *find_key(sat_t *satA, sat_t *satB);
void update_link_cap(GList *sats, GHashTable *history, gdouble limit, gdouble time, gdouble time_step);
...
void get_all_link_capacities(
    ...
    */
    ...
    for (gdouble time = t_start; time < t_end; time += (time_step / xmnpda)) {
        ...
        update_link_cap(mut_sats, linkCapBuild, link_cap_limit, time, time_step);
    }
    ...
    for (guint i = 0; i < *rem_len; i++){
        ...
        last_simpson_entry(linkCapBuild, keys[i], val, 0, t_end, time_step);
    }
    ...
    for (current = sats; current != NULL; current = current->next) {
        ...
        for (check = current->next; check != NULL; check = check->next) {
            ...
            if (tfr_value  == NULL && cur_value > limit) {
                ...
                last_simpson_entry(history, tfr_key, tfr_value, cur_value, time, time_step);
            }
...
void last_simpson_entry(
    ...
    printf("transfer key: %s, time:(%f -- %f) capacity (gb): %f\n", key, val->start_time, time, val->cumulative);
