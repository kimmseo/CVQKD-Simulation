...
gdouble get_transfer_time(
    ...
    gdouble skr = scaled_inter_sat_link(&src_sat_history[i], &dst_sat_history[i], time_step);
    
    //i = 1
    gint next_i = start_i + 1;
    if (next_i >= history_len) return G_MAXDOUBLE;
    gdouble next_skr = scaled_inter_sat_link(&src_sat_history[next_i], &dst_sat_history[next_i], time_step);
    ...
    while(check_term < data_size) { 
        ...
        next_i++;
        next_skr = scaled_inter_sat_link(&src_sat_history[next_i], &dst_sat_history[next_i], time_step);
...
gdouble accum_pre_start(gdouble x_i_mid, gint start_i, gdouble t_start, gdouble time_step, vector_t *src_hist, vector_t *dst_hist) {
    ...
    gdouble y_i = scaled_inter_sat_link(&src_hist[start_i], &dst_hist[start_i], time_step);
    gdouble x_i_prev = x_i - time_step;
    gdouble y_i_prev = scaled_inter_sat_link(&src_hist[start_i - 1], &dst_hist[start_i - 1], time_step);
...
gdouble accum_post_end(gdouble data_size, gint start_i, gdouble t_start, gdouble t_end, gdouble time_step, vector_t *src_hist, vector_t *dst_hist) {
    ...
    gdouble x_i = t_start  + (start_i * time_step);
    gdouble y_i = scaled_inter_sat_link(&src_hist[start_i], &dst_hist[start_i], time_step);
    gdouble y_i_nxt = scaled_inter_sat_link(&src_hist[start_i+1], &dst_hist[start_i+1], time_step);
