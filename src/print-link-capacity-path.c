...
//TESTING FILE. Can remove when link capacities has been implemented to satisfactory level

static void sats_at_time(GList *sats, gdouble time_step);
static void save_sat_close_approach(GList *sats, gdouble limit, GHashTable *history);
static char* save_transfer_info(gpointer satA_ptr, gpointer satB_ptr, gdouble skr);
static gpointer copy_glist_element(gconstpointer src, gpointer user_data);
...
void print_transfer_connections(GHashTable *sats, double t_start, double t_end, double time_step) {
    ...
    //keys are int1 * 1000 + int2, value is gstring formatted to print
...
static void sats_at_time(GList *sats, gdouble time) {
    GList *current;
